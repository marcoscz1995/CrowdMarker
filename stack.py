...
class Worker(threading.Thread):
    def __init__(self, comments, max_score,
                 next_booklet_key, perfect_score_key):
...
if __name__ == "__main__":
    ...
    NEXT_PAGE = 'f'
    PERFECT_SCORE = 'g'
    user_input = [[r"show what integration technique used here", -1, 'a'],
                  [r"your integrand bounds for $r$ are for $\theta$. need to swap them.", -1, 'r']]

    comments = Comment(user_input)
    worker = Worker(comments.comments, MAX_SCORE, NEXT_PAGE, PERFECT_SCORE)
    worker.start()
